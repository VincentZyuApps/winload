...
winload --graph-style line --x-axis 5s --y-axis unit # Rust：折線圖，具時流二軸
...
| `-U`, `--unicode` | 惟 `classic` 圖用 Unicode 方塊字（█▓░·） | 關 |
| `--graph-style <STYLE>` | **[惟 Rust]** 圖式：`classic`、`line`、`scatter`、`bar` | `classic` |
| `--x-axis <MODE>` | **[惟 Rust]** X 軸網格：`none` 或 `5s` 等正整秒間 | `none` |
| `--y-axis <MODE>` | **[惟 Rust]** Y 軸標及網格：`none`、`percent`、`unit` | `none` |
...
| `=` | 切分隔線顯隱 |
| `c` | 切色開關 |
| `g` | **[惟 Rust]** 循 `classic` → `line` → `scatter` → `bar` |
| `x` | **[惟 Rust]** 切時軸；未定其間則用 `5s` |
| `y` | **[惟 Rust]** 循 Y 軸：`none` → `percent` → `unit` |
| `q` / `Esc` | 退 |

> **Rust 圖控：** `classic` 為默。`g`、`x`、`y` 惟改當次運行；四圖式共用 X/Y 網格與縮放視域。`--unicode` 惟效於 `classic`，Ratatui 諸式用其原生標記。
