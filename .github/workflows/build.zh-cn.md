...
CI/CD 流水线完全由 **commit 信息中的方括号标签** 驱动。推送到 `main` 分支时，只需在 commit message 中包含对应标签，GitHub Actions 会自动完成后续工作。
...
> **触发格式：** 只有完整的方括号标签（如 `[build-release]`）才会触发 CI。裸关键词、圆括号关键词和相近标签均不生效；组合任务请分别使用标签，例如 `[build-publish] [pypi-publish]`。
